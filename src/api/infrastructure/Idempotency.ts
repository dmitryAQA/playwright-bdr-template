 ...
 * (Architect Pattern: Infrastructure Layer)
 *
...
export function generateIdempotencyKey(method: string, url: string, data: any): string {
    ...
    const stableStringify = (obj: any) => JSON.stringify(obj, obj ? Object.keys(obj).sort() : undefined);

    const payload = `${method}:${url}:${stableStringify(data)}`;

    return createHash('sha256').update(payload).digest('hex').slice(0, 16); // Short hash is usually enough for test isolation
}
